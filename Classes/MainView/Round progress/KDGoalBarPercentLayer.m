...
-(void)drawInContext:(CGContextRef)ctx {
    ...
    CGPoint center = CGPointMake(self.frame.size.width / (2) - 40, self.frame.size.height / (2));

    CGFloat delta = toRadians(360 * percent);
    
    CGFloat innerRadius = 62;
    CGFloat outerRadius = 70;
    
    if (color) {
        CGContextSetFillColorWithColor(ctx, color.CGColor);
    } else {
        CGContextSetFillColorWithColor(ctx, [UIColor colorWithRed:255/256.0 green:170/256.0 blue:0/256.0 alpha:.5].CGColor);
    }
