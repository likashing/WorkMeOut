...
//    [APPDELEGATE getTraineeProgramID];
//    
//    [self dataRequestNewActivity];
...
- (IBAction)onCreateSchedule:(id)sender {
    ...
    if ([_txtProgram.text isEqual:@""] || [_txtDifficulty.text isEqual:@""] || [_txtFrequency.text isEqual:@""] || [_txtType.text isEqual:@""] || [_txtViewProgram.text isEqual:@""]) {
        
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Notes!" message:@"Please enter the Program Data." delegate:nil cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
        [alert show];
...
- (void) dataRequestNewActivity
{
    bAdd_CreateProgram = true;
    ...
    NSDictionary *dict = [[NSDictionary alloc] initWithObjectsAndKeys: str_id, @"user_id", @"", @"trainee_program_id", nil];
    
    NSLog(@"%@", dict);
    
    
    APPDELEGATE.serverCall.delegate = self;
    [APPDELEGATE.serverCall requestServer:dict url:@"new_free_activity"];

}


- (void) dataRequestCreateProgram {
...
- (void) OnConnectFail
{
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Fail!" message:@"Add Workout program failure!" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
    [alert show];
}
