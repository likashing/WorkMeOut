...
#import "SuplementsViewController.h"
#import "DirectoryCollectionViewCell.h"
#import "StoreViewController.h"



@interface SuplementsViewController ()
{
    NSInteger index;
}
@end

@implementation SuplementsViewController
...
- (void) dataRequest
{
    ...
    NSString *str_type = @"store"; // Enter Directory`s ID
...
- (void) OnReceived:(NSDictionary *)dictData
{
    ...
    if (_arr_directories.count == 0) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Note!" message:@"No Supplement Stores yet." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
        [alert show];
    }
...
{
    [SVProgressHUD dismiss];
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Fail!" message:@"Store directories getting failure!" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
    [alert show];
